...
int WIN_CreateWindowFramebuffer(_THIS, SDL_Window * window, Uint32 * format, void ** pixels, int *pitch)
{
    ...
    *format = SDL_PIXELFORMAT_UNKNOWN;
    if (info->bmiHeader.biCompression == BI_BITFIELDS) {
        ...
        //*format = SDL_MasksToPixelFormatEnum(bpp, masks[0], masks[1], masks[2], 0xFF000000);
		*format = SDL_MasksToPixelFormatEnum(bpp, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000);
    }
...
int WIN_UpdateWindowFramebuffer(_THIS, SDL_Window * window, SDL_Rect * rects, int numrects)
{
    ...
    //BitBlt(data->hdc, 0, 0, window->w, window->h, data->mdc, 0, 0, SRCCOPY);
	{
		...
		//GetWindowRect(data->hwnd, &rcWindow);
		//GetClientRect(data->hwnd, &clientRect);
		//szDest.cx = clientRect.right-clientRect.left;
		//szDest.cy = clientRect.bottom-clientRect.top;
		//ptDest.x = rcWindow.left;
		//ptDest.y = rcWindow.top;
		
		blend_func.BlendOp = AC_SRC_OVER;
		blend_func.BlendFlags = 0;
		blend_func.SourceConstantAlpha = (unsigned char)(255*window->alpha);
		blend_func.AlphaFormat = AC_SRC_ALPHA;

		ret = UpdateLayeredWindow(data->hwnd,NULL,&ptDest,&szDest,data->mdc,&ptSrc,0,&blend_func,ULW_ALPHA);
